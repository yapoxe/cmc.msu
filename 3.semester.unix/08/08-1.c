/*
На стандартный поток ввода подается содержимое файла, описывающего структуру 32-битного виртуального адресного пространства процесса. 
...
{
    unsigned int sum = 0;
    char str[PATH_MAX + PREFIX];
    unsigned int start;
    unsigned int end;
    while(fgets(str, sizeof(str), stdin) != NULL) {
        sscanf(str,"%x-%x", &start, &end);
        sum += (end - start);
    }
    printf("%u\n", sum);
    return 0;
